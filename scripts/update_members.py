#!/usr/bin/env python3
"""Update member data files and regenerate HTML member tables from JSON.

This tool is used by `sync_from_sheets.py` to process member data synced from
Google Sheets. It consumes the JSON file generated by the sync script and
rewrites the corresponding HTML tables in `about/members.md`. It also provides
a function to convert the JSON data to an Excel spreadsheet.
...
Usage
-----
Typically called by sync_from_sheets.py, but can be run standalone:
...
def update_members_md():
    """Update the Markdown file with member tables from the JSON data.

    Reads members.json, processes entries by status, generates HTML table bodies,
    and updates about/members.md with the new tables. Also updates the last_updated
